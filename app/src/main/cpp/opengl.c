...
Java_com_yaopb_gifdemo_GifHandler_openFile(JNIEnv *env, jobject instance, jstring path_) {
    return openFile(env, path_);
}

JNIEXPORT jint JNICALL
Java_com_yaopb_gifdemo_GifHandler_getWidth__J(JNIEnv *env, jobject instance, jlong gifinfo) {
    return getWidth(gifinfo);
}

JNIEXPORT jint JNICALL
Java_com_yaopb_gifdemo_GifHandler_getHeight__J(JNIEnv *env, jobject instance, jlong gifinfo) {
    return getHeight(gifinfo);
}

JNIEXPORT jlong JNICALL
Java_com_yaopb_gifdemo_GifHandler_renderFrame(JNIEnv *env, jobject instance, jlong gifinfo,
                                              jobject bitmap) {
    GifInfo *info = (GifInfo *) (intptr_t) gifinfo;
    if (info == NULL)
        return -1;
    ...
    long renderStartTime = getRealTime();
    void *pixels;
    if (lockPixels(env, bitmap, info, &pixels) != 0) {
        return 0;
    }
    DDGifSlurp(info, true, false);
    if (info->currentIndex == 0) {
